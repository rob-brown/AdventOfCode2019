...
pub fn solve() {
    ...
    let mut panels: HashMap<(i32, i32), i32> = HashMap::new();
    ...
    while machine.halted == false {
        let color = panels.get(&current).unwrap_or(&0);

        machine.run_intcode(vec![*color as i64]);

        let color = machine.values.pop().unwrap();
        let turn = machine.values.pop().unwrap();

        let (dir, point) = match (direction, turn) {
            ("up", 0) => ("left", (current.0 - 1, current.1)),
            ("up", 1) => ("right", (current.0 + 1, current.1)),
            ("left", 0) => ("down", (current.0, current.1 + 1)),
            ("left", 1) => ("up", (current.0, current.1 - 1)),
            ("down", 0) => ("right", (current.0 + 1, current.1)),
            ("down", 1) => ("left", (current.0 - 1, current.1)),
            ("right", 0) => ("up", (current.0, current.1 - 1)),
            ("right", 1) => ("down", (current.0, current.1 + 1)),
            _ => panic!(),
        };

        panels.insert(current, color as i32);
